...
#else
    const char* pszModule = "Truckcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Truckcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Truckcoin
    // Mac: ~/Library/Application Support/Truckcoin
    // Unix: ~/.Truckcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Truckcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Truckcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Truckcoind.pid"));
